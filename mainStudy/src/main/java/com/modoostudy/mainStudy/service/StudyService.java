...
//    private final EntityManager em = emf.createEntityManager();
    ...
    public ReadStudyListDto getStudyList() {
        ...
        if (authentication.getPrincipal().toString().contentEquals("anonymousUser")) {
            ...
            //자체 카테고리 선정
            //studyID 8번을 운영자로 지칭하고 해당 유저의 interest를 따라감
            List<MappingUserInterest> mappingUserInterestsList = userRepository.findByUserID(8L).getUserInterests();
            List<StudyDto.ReadStudyList> studyList = getStudyListHelp(mappingUserInterestsList);

            return ReadStudyListDto.builder()
                    .readStudyListDto(studyList)
                    .build();
        }
    ...
    /*
    스터디 리스트보기 if/else문 중복을 막기위해 생성
    MappingUserInterest -> List<StudyDto.ReadStudyList>
     */
    public List<StudyDto.ReadStudyList> getStudyListHelp(List<MappingUserInterest> mappingUserInterestsList) {

        List<StudyDto.ReadStudyList> studyList = new ArrayList<>();
        for (MappingUserInterest userInterest : mappingUserInterestsList) {

            String userCategory = userInterest.getInterest().getInterestName();

            for (MappingStudyInterest studyInterest : mappingStudyInterestRepository.findByInterest(userInterest.getInterest())) {

                List<String> interestNameList = studyInterest.getStudy().getStudyInterests().stream()
                        .map(mappingStudyInterest -> mappingStudyInterest.getInterest())
                        .map(Interest::getInterestName)
                        .collect(Collectors.toList());

                studyList.add(StudyDto.ReadStudyList.builder()
                        .title(studyInterest.getStudy().getTitle())
                        .category(userCategory)
                        .needPeople(studyInterest.getStudy().getNeedPeople())
                        .periodStart(studyInterest.getStudy().getPeriodStart())
                        .periodEnd(studyInterest.getStudy().getPeriodEnd())
                        .onoffline(studyInterest.getStudy().getOnoffline())
